-- Adiciona URLs publicas de imagens geradas via Supabase Storage.
ALTER TABLE casa
    ADD COLUMN IF NOT EXISTS foto_fachada_url TEXT,
    ADD COLUMN IF NOT EXISTS foto_detalhe_url TEXT;
...
BEGIN
    ...
    INSERT INTO casa (id_setor, coordenada_lat, coordenada_long, logradouro, numero, bairro, observacao, cep, tipo_construcao, uso_imovel, status_imovel, data_interdicao, foto_fachada_url, foto_detalhe_url)
    VALUES ((v_casa->>'id_setor')::INTEGER, (v_casa->>'coordenada_lat')::DECIMAL, (v_casa->>'coordenada_long')::DECIMAL,
        ...
        NULLIF(v_casa->>'foto_fachada_url', ''), NULLIF(v_casa->>'foto_detalhe_url', ''))
    ON CONFLICT (id_setor, bairro, logradouro, numero) DO UPDATE SET
        foto_fachada_url = COALESCE(EXCLUDED.foto_fachada_url, casa.foto_fachada_url),
        foto_detalhe_url = COALESCE(EXCLUDED.foto_detalhe_url, casa.foto_detalhe_url)
    ...
    FOR v_membro IN SELECT value FROM jsonb_array_elements(p_payload->'individuos') LOOP
        ...
        END IF;
        INSERT INTO individuo (id_nucleo_familiar, nome_completo, apelido, nome_social, data_nascimento, genero, cor_raca, uf, estado_civil, profissao, nome_mae, nome_pai, grau_parentesco, escolaridade, situacao_ocupacional, cpf, doc_estrangeiro, rg, nis, telefone, email, status_vital, data_obito, semanas_gestacao, foto_url)
        VALUES (v_nucleo_id, UPPER(BTRIM(v_membro->>'nome_completo')), NULLIF(v_membro->>'apelido',''), NULLIF(v_membro->>'nome_social',''),
            ...
            NULLIF(v_membro->>'semanas_gestacao','')::INTEGER, NULLIF(v_membro->>'foto_url','')) RETURNING id INTO v_individuo_id;
