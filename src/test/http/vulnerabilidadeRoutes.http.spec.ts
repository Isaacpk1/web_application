...
jest.mock('../../repositories/VulnerabilidadeRepository', () => ({
  VulnerabilidadeRepository: jest.fn().mockImplementation(() => ({
    findAll: jest.fn().mockResolvedValue([mockVulnerabilidade]),
    findById: jest.fn((id: number) => Promise.resolve(id === 99999 || id === 99998 ? null : { ...mockVulnerabilidade, id })),
    findByIndividuoId: jest.fn().mockResolvedValue([mockVulnerabilidade]),
    create: jest.fn((payload) => Promise.resolve({ ...mockVulnerabilidade, ...payload, id: 6 })),
    update: jest.fn((id, payload) => Promise.resolve({ ...mockVulnerabilidade, ...payload, id })),
    delete: jest.fn().mockResolvedValue(undefined),
    associate: jest.fn((payload) => Promise.resolve(payload)),
